...
const ProductShowcase = ({product, initialColor}: ProductShowcaseProps) => {
  ...
  const getOriginalPrice = () => {
    return getCurrentVariant()?.price || product.basePrice
  }
  ...
  const handleAddToCart = () => {
    ...
    addItem({
      ...
      price: Math.round(variant.price * 100), // Keep original price
          ...
          <div className="space-y-6 fade-up">
            <div className="space-y-4">
              ...
              <div className="space-y-1">
                {saleActive && (
                  <div className="inline-block bg-black text-white text-xs font-bold px-2 py-1 rounded">
                    {discountPercent}% OFF
                  </div>
                )}
                <div className="flex items-center gap-3">
                  <p className={`text-2xl font-medium ${saleActive ? 'text-black dark:text-white' : 'text-black dark:text-white'}`}>
